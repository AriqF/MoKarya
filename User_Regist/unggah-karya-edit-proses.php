<?php
    ...
    if($foto_karya != "") {
      ...
      $nama_gambar_baru = $angka_acak.'-'.$foto_karya; //menggabungkan angka acak dengan nama file sebenarnya
          if(in_array($ekstensi, $ekstensi_diperbolehkan) === true && $width === 1920 && $height === 1080)  {     
                  move_uploaded_file($file_tmp, 'gambar/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                    ...
                    if(!$result){
                      ...
                      echo "<script>alert('Data berhasil diubah.');window.location='unggah-karya-data';</script>";
                    }
            } 
            elseif($width != 1920 || $height != 1080){
                echo "<script>alert('Gambar harus memiliki dimensi 1920x1080!');window.location='unggah-karya-data';</script>";
              }
            else {     
            //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='unggah-karya-edit';</script>";
            }
    } 
    else {
        ...
        // periska query apakah ada error
        if(!$result){
          echo "<script>alert('Upload gagal');window.location='unggah-karya-data';</script>";
            die ("Query gagal dijalankan: ".mysqli_errno($conn).
                " - ".mysqli_error($conn));
        } else {
          //tampil alert dan akan redirect ke halaman index.php
